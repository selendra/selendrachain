...
pub use indracore_parachain::primitives::{
    BlockData, HeadData, HrmpChannelId, Id, UpwardMessage, ValidationCode, LOWEST_USER_ID,
};

// Export some basic parachain primitives from v0.
pub use crate::v0::{
    CollatorId, CollatorSignature, CompactStatement, EncodeAs, ErasureChunk, Signed,
    SignedStatement, SigningContext, ValidatorId, ValidatorIndex, ValidatorSignature,
...
impl CompressedPoV {
    ...
    pub fn compress(pov: &PoV) -> Result<Self, CompressedPoVError> {
        zstd::encode_all(pov.encode().as_slice(), 3)
            .map_err(|_| CompressedPoVError::Compress)
            .map(Self)
    }
    ...
    pub fn decompress(&self) -> Result<PoV, CompressedPoVError> {
        ...
        let mut decoder =
            zstd::Decoder::new(self.0.as_slice()).map_err(|_| CompressedPoVError::Decompress)?;
...
impl ConsensusLog {
    ...
    pub fn from_digest_item<H>(
        ...
        match digest_item {
            runtime_primitives::DigestItem::Consensus(id, encoded)
                if id == &INDRACORE_ENGINE_ID =>
            {
...
mod tests {
    ...
    fn decompress_huge_pov_block_fails() {
        let pov = PoV {
            block_data: vec![0; 63 * 1024 * 1024].into(),
        };

        let compressed = CompressedPoV::compress(&pov).unwrap();
        assert_eq!(
            CompressedPoVError::Decode,
