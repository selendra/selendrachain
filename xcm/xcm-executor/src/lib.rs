...
use frame_support::{dispatch::Dispatchable, ensure};
use parity_scale_codec::Decode;
use sp_std::{convert::TryInto, marker::PhantomData, prelude::*};
use xcm::v0::{
    Error as XcmError, ExecuteXcm, Junction, MultiAsset, MultiLocation, Order, Result as XcmResult,
    SendXcm, Xcm,
};
...
use traits::{ConvertOrigin, FilterAssetLocation, InvertLocation, TransactAsset};
        ...
        let (mut holding, effects) = match (origin.clone(), msg) {
            (origin, Xcm::RelayedFrom { superorigin, inner }) => {
                ...
                return Self::execute_xcm(
                    ...
                    (*inner)
                        .try_into()
                        .map_err(|_| XcmError::UnhandledXcmVersion)?,
                );
            }
            ...
            (origin, Xcm::ReserveAssetDeposit { assets, effects }) => {
                ...
                if assets
                    .iter()
                    .all(|asset| Config::IsReserve::filter_asset_location(asset, &origin))
                {
            ...
            (origin, Xcm::Transact { origin_type, call }) => {
                ...
                let message_call =
                    Config::Call::decode(&mut &call[..]).map_err(|_| XcmError::FailedToDecode)?;
            ...
            (
                origin,
                Xcm::RelayTo {
                    dest: MultiLocation::X1(Junction::Parachain { id }),
                    inner,
                },
            ) => {
