...
fn collect_relay_storage_proof(
    indracore_backend: &impl Backend<PBlock>,
    para_id: ParaId,
    relay_parent: PHash,
) -> Option<sp_state_machine::StorageProof> {
    use relay_chain::well_known_keys as relay_well_known_keys;

    let relay_parent_state_backend = indracore_backend
...
impl ParachainInherentData {
        ...
        PClient::Api: ParachainHost<PBlock>,
    {
        let relay_chain_state =
            collect_relay_storage_proof(indracore_backend, para_id, relay_parent)?;
        let downward_messages = retrieve_dmq_contents(indracore_client, para_id, relay_parent)?;
        let horizontal_messages =
            retrieve_all_inbound_hrmp_channel_contents(indracore_client, para_id, relay_parent)?;
