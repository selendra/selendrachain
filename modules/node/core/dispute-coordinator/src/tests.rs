...
use super::*;
use selendra_primitives::v1::{BlakeTwo256, HashT, ValidatorId, Header, SessionInfo};
use selendra_node_subsystem::{jaeger, ActiveLeavesUpdate, ActivatedLeaf, LeafStatus};
...
struct TestState {
	...
	polkadot-v0.9.8_keystore: Arc<sc_keystore::LocalKeystore>,
...
impl Default for TestState {
	fn default() -> TestState {
		...
		let polkadot-v0.9.8_keystore = make_keystore(&validators).into();
...
impl TestState {
	...
	async fn issue_statement_with_index(
		...
		let keystore = self.polkadot-v0.9.8_keystore.clone() as SyncCryptoStorePtr;
...
fn supermajority_valid_dispute_may_be_finalized() {
	test_harness(|test_state, mut virtual_overseer| Box::pin(async move {
		...
		let supermajority_threshold = selendra_primitives::v1::supermajority_threshold(
