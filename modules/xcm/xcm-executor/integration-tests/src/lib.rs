...
fn basic_buy_fees_message_executes() {
	...
	let execute = construct_extrinsic(
		&client,
		selendra_test_runtime::Call::Xcm(pallet_xcm::Call::execute {
	...
	client
		...
		.inspect_state(|| {
			assert!(selendra_test_runtime::System::events().iter().any(|r| matches!(
				r.event,
				selendra_test_runtime::Event::Xcm(pallet_xcm::Event::Attempted(Outcome::Complete(
					_
				))),
			)));
		});
...
fn query_response_fires() {
	...
	use selendra_test_runtime::Event::TestNotifier;
	...
	let execute = construct_extrinsic(
		&client,
		selendra_test_runtime::Call::TestNotifier(pallet_test_notifier::Call::prepare_new_query {}),
		sp_keyring::Sr25519Keyring::Alice,
		0,
	);
	...
	client
		...
		.inspect_state(|| {
			assert!(selendra_test_runtime::System::events().iter().any(|r| matches!(
				r.event,
				selendra_test_runtime::Event::Xcm(pallet_xcm::Event::ResponseReady(
					q,
...
fn query_response_elicits_handler() {
	...
	let execute = construct_extrinsic(
		&client,
		selendra_test_runtime::Call::TestNotifier(
