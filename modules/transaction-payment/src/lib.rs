...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::getter(fn transaction_tracer)]
	pub type TransactionTracer<T: Config> =
		StorageMap<_, Twox64Concat, T::AccountId, u16, ValueQuery>;
	...
	BalanceOf<T>: Send + Sync + FixedPointOperand,
{
	...
	> {
		let user_call = <TransactionTracer<T>>::get(&who);
		...
		if user_call < 5 {
			ensure_free = true;
			fee = self.0;
			TransactionTracer::<T>::insert(who, user_call.saturating_add(1));
		}

		<<T as Config>::OnChargeTransaction as OnChargeTransaction<T>>::withdraw_fee(
			who, call, info, fee, tip, ensure_free
		)
		.map(|i| (fee, i))
	}
	...
	T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	fn validate(
		...
		let (final_fee, _) = self.withdraw_fee(who, call, info, len, false)?;
		let tip = self.0;
		Ok(ValidTransaction {
	...
	fn pre_dispatch(
		...
		let (_fee, imbalance) = self.withdraw_fee(who, call, info, len, false)?;
