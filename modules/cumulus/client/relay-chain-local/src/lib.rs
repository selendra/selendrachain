...
) -> Result<(NewFull<selendra_client::Client>, CollatorPair), selendra_service::Error> {
...
) -> Result<(Arc<(dyn RelayChainInterface + 'static)>, CollatorPair), selendra_service::Error> {
	let (full_node, collator_key) =
		build_selendra_full_node(selendra_config, telemetry_worker_handle).map_err(
			|e| match e {
