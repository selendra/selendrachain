...
use sp_runtime::traits::{
    AccountIdConversion, CheckedConversion, CheckedSub, Hash, One, StaticLookup, Zero,
};
use sp_std::{convert::TryInto, mem::swap, prelude::*};
...
type WinnersData<T> = Vec<(
    Option<NewBidder<<T as frame_system::Config>::AccountId>>,
    ParaId,
    BalanceOf<T>,
    SlotRange,
)>;
...
impl<T: Config> Module<T> {
        ...
        if let Some((lease_period_index, early_end)) = <AuctionInfo<T>>::get() {
            ...
            if early_end + ending_period == now {
                ...
                let offset = T::BlockNumber::decode(&mut T::Randomness::random_seed().as_ref())
                    .expect("secure hashes always bigger than block numbers; qed")
                    % ending_period;
        ...
        for (maybe_new_deploy, para_id, amount, range) in winners.into_iter() {
            match maybe_new_deploy {
                Some(bidder) => {
                    ...
                    Self::deposit_event(RawEvent::WonDeploy(
                        bidder.clone(),
                        range,
                        para_id,
                        amount,
    ...
    fn manage_lease_period_start(lease_period_index: LeasePeriodOf<T>) {
                    ...
                    if !d.is_empty() {
                        ...
                        if d.len() == 1 {
                            ...
                            if let Some(rebate) = outgoing.checked_sub(&new_held) {
                                T::Currency::deposit_creating(&id.into_account(), rebate);
                            }
        ...
        for para_id in <OnboardQueue<T>>::take(lease_period_index) {
            ...
            )) = <Onboarding<T>>::get(&para_id)
            {
                ...
                let _ =
                    T::Parachains::register_para(para_id.clone(), true, code, initial_head_data);
    ...
    pub fn handle_bid(
        ...
        if current_winning[range_index]
            .as_ref()
            .map_or(true, |last| amount > last.1)
        {
            // This must overlap with all existing ranges that we're winning on or it's invalid.
                ...
                current_winning
                    .iter()
                    .enumerate()
                    .all(|(i, x)| x.as_ref().map_or(true, |(w, _)| w != &bidder
                        || range.intersects(i.try_into().expect(
    ...
    fn calculate_winners(
        ...
        let winning_ranges = {
            let mut best_winners_ending_at: [(Vec<SlotRange>, BalanceOf<T>); 4] =
                Default::default();
...
mod tests {
    ...
    fn set_offboarding_works() {
        new_test_ext().execute_with(|| {
            ...
            run_to_block(49);
            assert_eq!(Slots::deposit_held(&0.into()), 1);
            assert_ok!(Slots::set_offboarding(
                Origin::signed(ParaId::from(0).into_account()),
                10
            ));
    ...
    fn onboarding_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(Slots::elaborate_deploy_data(
                Origin::signed(0),
                0.into(),
                vec![42].into()
            ));
    ...
    fn renewal_with_lower_value_should_work() {
        new_test_ext().execute_with(|| {
            ...
            run_to_block(20);
            assert_eq!(
                Balances::free_balance(&ParaId::from(0u32).into_account()),
                2
            );
    ...
    fn incomplete_calculate_winners_works() {
        ...
        let winners = vec![(
            Some(NewBidder { who: 1, sub: 0 }),
            0.into(),
            1,
            SlotRange::ThreeThree,
        )];

        assert_eq!(
            Slots::calculate_winners(winning, TestParachains::new_id),
            winners
        );
    }
    ...
    fn calculate_winners_works() {
        ...
        let winners = vec![
            (
                Some(NewBidder { who: 2, sub: 0 }),
                0.into(),
                2,
                SlotRange::ZeroZero,
            ),
            (
                Some(NewBidder { who: 3, sub: 0 }),
                1.into(),
                1,
                SlotRange::OneOne,
            ),
        ...
        assert_eq!(
            Slots::calculate_winners(winning.clone(), TestParachains::new_id),
            winners
        );

        reset_count();
        winning[SlotRange::ZeroThree as u8 as usize] =
            Some((Bidder::New(NewBidder { who: 1, sub: 0 }), 2));
        let winners = vec![
            (
                Some(NewBidder { who: 2, sub: 0 }),
                0.into(),
        ...
        reset_count();
        winning[SlotRange::ZeroOne as u8 as usize] =
            Some((Bidder::New(NewBidder { who: 4, sub: 0 }), 3));
        let winners = vec![
            (
                Some(NewBidder { who: 4, sub: 0 }),
                0.into(),
                3,
                SlotRange::ZeroOne,
            ),
    ...
    fn code_size_must_be_correct() {
        new_test_ext().execute_with(|| {
            ...
            assert!(
                Slots::elaborate_deploy_data(Origin::signed(0), 0.into(), code.into()).is_err()
            );
        });
    }
}
