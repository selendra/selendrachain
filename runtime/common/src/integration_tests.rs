...
use primitives::v1::{BlockNumber, HeadData, Header, Id as ParaId, ValidationCode};
...
fn basic_end_to_end_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Paras::lifecycle(ParaId::from(1)),
            Some(ParaLifecycle::Onboarding)
        );
        assert_eq!(
            Paras::lifecycle(ParaId::from(2)),
            Some(ParaLifecycle::Onboarding)
        );
        ...
        run_to_block(110);
        assert_eq!(last_event(), auctions::RawEvent::AuctionClosed(1).into(),);
...
fn basic_swap_works() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // Cant dissolve
        assert_noop!(
            Crowdloan::dissolve(Origin::signed(1), ParaId::from(1)),
            CrowdloanError::<Test>::InvalidParaId
        );
        assert_noop!(
            Crowdloan::dissolve(Origin::signed(2), ParaId::from(2)),
