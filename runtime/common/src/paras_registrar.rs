...
mod tests {
    ...
    use runtime_parachains::{
        configuration, dmp, hrmp, inclusion, initializer, scheduler, session_info, shared, ump,
        ParaLifecycle,
    };
    use sp_core::H256;
    ...
    fn new_test_ext() -> TestExternalities {
        ...
        let balances: Vec<_> = (0..authority_keys.len())
        ...
        let session_keys: Vec<_> = authority_keys
            .iter()
            .enumerate()
    ...
    fn register_deregister_chain_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(Registrar::enable_parathread_registration(Origin::root(),));
            run_to_block(2);
            ...
            assert_eq!(
                Balances::free_balance(3u64) + ParathreadDeposit::get(),
                orig_bal
            );
    ...
    fn swap_handles_funds_correctly() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(Registrar::swap(
                runtime_parachains::Origin::Parachain(2u32.into()).into(),
                8u32.into()
            ));
            assert_ok!(Registrar::swap(
                runtime_parachains::Origin::Parachain(8u32.into()).into(),
