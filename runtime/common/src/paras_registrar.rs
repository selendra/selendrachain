...
impl<T: Config> Registrar for Module<T> {
    ...
    fn make_parachain(id: ParaId) -> DispatchResult {
        // Para backend should think this is a parathread...
            ...
            paras::Module::<T>::lifecycle(id) == Some(ParaLifecycle::Parathread),
    ...
    fn make_parathread(id: ParaId) -> DispatchResult {
        // Para backend should think this is a parachain...
            ...
            paras::Module::<T>::lifecycle(id) == Some(ParaLifecycle::Parachain),
    ...
    fn execute_pending_transitions() {
        ...
        shared::Module::<T>::set_session_index(shared::Module::<T>::scheduled_session());
...
impl<T: Config> Module<T> {
    ...
    fn do_register(
        ...
        validation_code: ValidationCode,
            ...
            paras::Module::<T>::lifecycle(id).is_none(),
    ...
    fn validate_onboarding_data(
        ...
        parachain: bool,
            ...
            validation_code.0.starts_with(WASM_MAGIC),
            Error::<T>::DefinitelyNotWasm
        );
...
mod tests {
    ...
    fn end_to_end_scenario_works() {
        new_test_ext().execute_with(|| {
            ...
            assert!(!Parachains::is_parachain(32.into()));
            // Deregister it
            assert_ok!(Registrar::deregister(Origin::root(), 32.into(),));
            run_to_session(8);
            // It is nothing
            assert!(!Parachains::is_parathread(32.into()));
            assert!(!Parachains::is_parachain(32.into()));
    ...
    fn deregister_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            // Origin check
            assert_noop!(
                Registrar::deregister(Origin::signed(1), 32.into(),),
                BadOrigin
            );
            // not registered
            assert_noop!(
                Registrar::deregister(Origin::root(), 33.into(),),
            ...
            assert_noop!(
                Registrar::deregister(Origin::root(), 32.into(),),
                Error::<Test>::NotParathread
            );
        });
    }
    ...
    fn swap_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(Registrar::swap(para_origin(23.into()), 32.into()));
            assert_ok!(Registrar::swap(para_origin(32.into()), 23.into()));

            run_to_session(6);

            // Deregister a parathread that was originally a parachain
            assert_eq!(
                Parachains::lifecycle(23u32.into()),
                Some(ParaLifecycle::Parathread)
            );
            assert_ok!(Registrar::deregister(
                runtime_parachains::Origin::Parachain(23u32.into()).into(),
                23u32.into()
            ));
    ...
    fn cannot_register_until_para_is_cleaned_up() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                Parachains::lifecycle(1u32.into()),
                Some(ParaLifecycle::Parathread)
            );
