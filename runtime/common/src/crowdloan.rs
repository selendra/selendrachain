...
use frame_support::{
    ...
    storage::child,
    traits::{Currency, ExistenceRequirement::AllowDeath, Get, OnUnbalanced},
};
...
use sp_runtime::{
    traits::{
        AccountIdConversion, Bounded, CheckedAdd, Hash, IdentifyAccount, Saturating, Verify, Zero,
    },
    DispatchResult, ModuleId, MultiSignature, MultiSigner,
};
use sp_std::vec::Vec;
...
#[allow(dead_code)]
pub type NegativeImbalanceOf<T> = <<T as slots::Config>::Currency as Currency<
...
impl<T: Config> Module<T> {
    ...
    pub fn contribution_get(index: FundIndex, who: &T::AccountId) -> BalanceOf<T> {
        who.using_encoded(|b| child::get_or_default::<BalanceOf<T>>(&Self::id_from_index(index), b))
    }
...
mod tests {
    ...
    fn contribute_with_verifier_works() {
        new_test_ext().execute_with(|| {
            ...
            // Missing signature
            assert_noop!(
                Crowdloan::contribute(Origin::signed(1), 0, 49, None),
            ...
            // Invalid signature
            assert_noop!(
                Crowdloan::contribute(Origin::signed(1), 0, 49, Some(invalid_signature)),
                Error::<Test>::InvalidSignature
            );

            // Valid signature wrong parameter
            assert_noop!(
                Crowdloan::contribute(Origin::signed(1), 0, 50, Some(valid_signature.clone())),
                Error::<Test>::InvalidSignature
            );
            assert_noop!(
                Crowdloan::contribute(Origin::signed(2), 0, 49, Some(valid_signature.clone())),
                Error::<Test>::InvalidSignature
            );
            ...
            assert_noop!(
                Crowdloan::contribute(Origin::signed(1), 0, 49, Some(valid_signature)),
                Error::<Test>::InvalidSignature
            );
    ...
    fn contribute_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::contribute(Origin::signed(1), 0, 9, None),
            ...
            assert_noop!(
                Crowdloan::contribute(Origin::signed(2), 0, 900, None),
    ...
    fn onboard_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::onboard(Origin::signed(1), 1, 0.into()),
                Error::<Test>::InvalidFundIndex
            );
            // Cannot onboard crowdloan without deploy data
            assert_noop!(
                Crowdloan::onboard(Origin::signed(1), 0, 0.into()),
                Error::<Test>::UnsetDeployData
            );
            ...
            assert_noop!(
                Crowdloan::onboard(Origin::signed(1), 0, 1.into()),
                SlotsError::<Test>::ParaNotOnboarding
            );
    ...
    fn begin_retirement_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                Slots::offboarding(ParaId::from(0)),
                Crowdloan::fund_account_id(0)
            );
    ...
    fn begin_retirement_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::begin_retirement(Origin::signed(1), 0),
            ...
            assert_noop!(
                Crowdloan::begin_retirement(Origin::signed(1), 1),
                Error::<Test>::InvalidFundIndex
            );
    ...
    fn withdraw_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::withdraw(Origin::signed(1337), 1, 0),
            ...
            assert_noop!(
                Crowdloan::withdraw(Origin::signed(1337), 2, 0),
                Error::<Test>::NoContributions
            );
            // Cannot withdraw from a non-existent fund
            assert_noop!(
                Crowdloan::withdraw(Origin::signed(1337), 2, 1),
                Error::<Test>::InvalidFundIndex
            );
        });
    }
    ...
    fn partial_dissolve_works() {
        ...
        ext.execute_with(|| {
            ...
            // Check current funds (contributions + deposit)
            assert_eq!(
                Balances::free_balance(Crowdloan::fund_account_id(0)),
                100 * 30 + 1
            );
        });
    ...
    fn dissolve_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::dissolve(Origin::signed(1), 1),
                Error::<Test>::InvalidFundIndex
            );
            // Cannot dissolve a fund in progress
            assert_noop!(
                Crowdloan::dissolve(Origin::signed(1), 0),
                Error::<Test>::InRetirementPeriod
            );
...
mod benchmarking {
    ...
    fn worst_validation_code<T: Config>() -> Vec<u8> {
        ...
        let _ = validation_code
            .splice(..crate::WASM_MAGIC.len(), crate::WASM_MAGIC.iter().cloned())
            .collect::<Vec<_>>();
        validation_code
    }
    ...
    fn setup_onboarding<T: Config>(
        ...
        let onboarding_data = (
            lease_period_index,
            crate::slots::IncomingParachain::Unset(crate::slots::NewBidder {
