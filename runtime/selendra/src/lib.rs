...
parameter_types! {
	...
	// 2 hour session, 15 minutes unsigned phase, 16 offchain executions.
	...
	fn filter(&self, c: &Call) -> bool {
		match self {
			...
			ProxyType::NonTransfer => matches!(
				...
				Call::Slots(..) | // Specifically omitting the entire XCM Pallet
				Call::BagsList(..)
			),
...
mod test_fees {
	...
	fn full_block_council_election_cost() {
		...
		assert!(cost_units > 150_000); // DOLLAR ~ new SEL ~ 10e10
	}
