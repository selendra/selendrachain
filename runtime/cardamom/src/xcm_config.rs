...
parameter_types! {
	/// The location of the CDM token, from the context of this chain. Since this token is native to this
	/// chain, we make it synonymous with it and thus it is the `Here` location, which means "equivalent to
	/// the context".
	pub const CdmLocation: MultiLocation = Here.into();
	/// The Cardamom network ID. This is named.
	pub const CardamomNetwork: NetworkId = NetworkId::Cardamom;
	/// Our XCM location ancestry - i.e. what, if anything, `Parent` means evaluated in our context. Since
	/// Cardamom is a top-level relay-chain, there is no ancestry.
...
pub type SovereignAccountOf = (
	...
	AccountId32Aliases<CardamomNetwork, AccountId>,
);

/// Our asset transactor. This is what allows us to interest with the runtime facilities from the point of
/// view of XCM-only concepts like `MultiLocation` and `MultiAsset`.
///
/// Ours is only aware of the Balances pallet, which is mapped to `CdmLocation`.
...
parameter_types! {
	...
	pub const CardamomForStatemine: (MultiAssetFilter, MultiLocation) = (Cardamom::get(), Parachain(1000).into());
	pub const CardamomForEncointer: (MultiAssetFilter, MultiLocation) = (Cardamom::get(), Parachain(1001).into());
}
pub type TrustedTeleporters =
	(xcm_builder::Case<CardamomForStatemine>, xcm_builder::Case<CardamomForEncointer>);
...
pub type LocalOriginToLocation = (
	...
	SignedToAccountId32<Origin, AccountId, CardamomNetwork>,
);
