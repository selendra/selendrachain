...
mod tests {
    use super::*;
    use crate::mock::{new_test_ext, Configuration, Initializer, Origin};
    ...
    fn config_changes_on_session_boundary() {
        new_test_ext(Default::default()).execute_with(|| {
            ...
            assert_eq!(
                <Configuration as Store>::PendingConfig::get(),
                Some(config.clone())
            );
    ...
    fn setting_pending_config_members() {
        new_test_ext(Default::default()).execute_with(|| {
            ...
            .unwrap();
            Configuration::set_acceptance_period(Origin::root(), new_config.acceptance_period)
                .unwrap();
            ...
            Configuration::set_preferred_dispatchable_upward_messages_step_weight(
                Origin::root(),
                new_config.preferred_dispatchable_upward_messages_step_weight,
            )
            .unwrap();
