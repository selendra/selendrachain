...
use crate::{configuration, dmp, hrmp, paras};
...
pub fn make_transient_validation_data<T: paras::Config + dmp::Config>(
    ...
    let can_upgrade_code = last_code_upgrade.map_or(true, |l| {
        l <= relay_parent_number && relay_parent_number.saturating_sub(l) >= freq
    });
