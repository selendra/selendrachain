...
use frame_support::{decl_error, decl_module, decl_storage, traits::Get, weights::Weight};
...
impl ParaLifecycle {
    ...
    pub fn is_outgoing(&self) -> bool {
        matches!(
            self,
            ParaLifecycle::OutgoingParathread | ParaLifecycle::OutgoingParachain
        )
    }
...
impl<N: Ord + Copy> ParaPastCodeMeta<N> {
    ...
    fn code_at(&self, para_at: N) -> Option<UseCodeAt<N>> {
        ...
        let replaced_after_pos = self
            .upgrade_times
            .iter()
            .position(|t| t.expected_at >= para_at);
...
impl<T: Config> Module<T> {
    ...
    pub(crate) fn schedule_para_initialize(id: ParaId, genesis: ParaGenesisArgs) -> Weight {
        ...
        let dup = UpcomingParas::mutate(|v| match v.binary_search(&id) {
            Ok(_) => true,
            Err(i) => {
                v.insert(i, id);
                false
    ...
    pub(crate) fn schedule_para_cleanup(id: ParaId) -> Weight {
        match ParaLifecycles::get(&id) {
            ...
            Some(ParaLifecycle::Parathread) => {
                ...
                OutgoingParas::mutate(|v| match v.binary_search(&id) {
            ...
            Some(ParaLifecycle::UpgradingToParachain) => {
                let upgrade_weight = UpcomingUpgrades::mutate(|v| match v.binary_search(&id) {
                    Ok(i) => {
                        v.remove(i);
                ...
                let outgoing_weight = OutgoingParas::mutate(|v| match v.binary_search(&id) {
                    Ok(_) => T::DbWeight::get().reads_writes(1, 0),
                    Err(i) => {
                        v.insert(i, id);
                        ParaLifecycles::insert(&id, ParaLifecycle::OutgoingParathread);
            ...
            Some(ParaLifecycle::DowngradingToParathread) => {
                let downgrade_weight = UpcomingDowngrades::mutate(|v| match v.binary_search(&id) {
                    Ok(i) => {
                        v.remove(i);
                        T::DbWeight::get().reads_writes(1, 1)
                    }
            ...
            | Some(ParaLifecycle::OutgoingParachain) => T::DbWeight::get().reads(1),
        }
    }
    ...
    pub(crate) fn schedule_parathread_upgrade(id: ParaId) -> Weight {
        ...
        let dup = UpcomingUpgrades::mutate(|v| match v.binary_search(&id) {
            Ok(_) => true,
            Err(i) => {
                v.insert(i, id);
                false
    ...
    pub(crate) fn schedule_parachain_downgrade(id: ParaId) -> Weight {
        ...
        let dup = UpcomingDowngrades::mutate(|v| match v.binary_search(&id) {
            Ok(_) => true,
            Err(i) => {
                v.insert(i, id);
                false
        ...
        if upgrade_applied_intermediate {
            ...
            match Self::past_code_meta(&id).code_at(at) {
                ...
                Some(UseCodeAt::ReplacedAt(replaced)) => {
                    <Self as Store>::PastCode::get(&(id, replaced))
                }
            }
        }
    }
...
mod tests {
    ...
    fn para_past_code_pruning_in_initialize() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                <Paras as Store>::PastCode::get(&(id, at_block)),
                Some(vec![1, 2, 3].into())
            );
    ...
    fn code_upgrade_applied_after_delay() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            let expected_at = {
                ...
                assert_eq!(
                    <Paras as Store>::FutureCode::get(&para_id),
                    Some(new_code.clone())
                );
                assert_eq!(Paras::current_code(&para_id), Some(vec![1, 2, 3].into()));
    ...
    fn full_parachain_cleanup_storage() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                <Paras as Store>::PastCode::get(&(para_id, 3)),
                Some(vec![1, 2, 3].into())
            );
    ...
    fn code_at_with_intermediate() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                Paras::validation_code_at(para_id, 1, None),
                Some(old_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 10, None),
                Some(old_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 100, None),
                Some(old_code.clone())
            );

            // intermediate before upgrade meant to be applied, falls back on current.
            assert_eq!(
                Paras::validation_code_at(para_id, 9, Some(8)),
                Some(old_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 10, Some(9)),
                Some(old_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 11, Some(9)),
                Some(old_code.clone())
            );

            // intermediate at or after upgrade applied
            assert_eq!(
                Paras::validation_code_at(para_id, 11, Some(10)),
                Some(new_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 100, Some(11)),
                Some(new_code.clone())
            );
    ...
    fn code_at_returns_up_to_end_of_acceptance_period() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                Paras::validation_code_at(para_id, 2, None),
                Some(old_code.clone())
            );
            assert_eq!(
                Paras::validation_code_at(para_id, 3, None),
                Some(new_code.clone())
            );
