...
use primitives::v1::{
    BackedCandidate, Header, SignedAvailabilityBitfields, INCLUSION_INHERENT_IDENTIFIER,
};
use sp_std::prelude::*;
...
fn limit_backed_candidates<T: Config>(
    ...
    if frame_system::Module::<T>::block_weight().total()
    ...
    mod inclusion_inherent_weight {
        ...
        fn weight_does_not_change_on_happy_path() {
            new_test_ext(MockGenesisConfig::default()).execute_with(|| {
                ...
                let post_info =
                    Call::<Test>::inclusion(signed_bitfields, backed_candidates, default_header())
        ...
        fn weight_changes_when_backed_candidates_are_truncated() {
            new_test_ext(MockGenesisConfig::default()).execute_with(|| {
                ...
                // execute the inclusion inherent
                let post_info =
                    Call::<Test>::inclusion(signed_bitfields, backed_candidates, header)
                        .dispatch_bypass_filter(None.into())
                        .unwrap();
