...
impl<T: Config> Module<T> {
    ...
    pub(crate) fn set_active_validators(active: Vec<ValidatorId>) {
        ActiveValidatorIndices::set((0..active.len()).map(|i| ValidatorIndex(i as _)).collect());
...
mod tests {
    ...
    fn sets_and_shuffles_validators() {
        ...
        new_test_ext(MockGenesisConfig::default()).execute_with(|| {
            let validators = Shared::initializer_on_new_session(1, [1; 32], &config, pubkeys);
            ...
            assert_eq!(Shared::active_validator_keys(), validators,);
