...
fn signed_ext_builder_cardamom(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> cardamom_runtime_exports::SignedExtra {
	use cardamom_runtime_exports::Runtime;
	(
		frame_system::CheckNonZeroSender::<Runtime>::new(),
		frame_system::CheckSpecVersion::<Runtime>::new(),
		frame_system::CheckTxVersion::<Runtime>::new(),
...
construct_runtime_prelude!(cardamom);
	...
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				...
				$crate::AnyRuntime::Cardamom => {
					#[allow(unused)]
					use $crate::cardamom_runtime_exports::*;
					$($code)*
				}
			}
		}
	...
	match chain.to_lowercase().as_str() {
		"selendra" | "development" => {
			sp_core::crypto::set_default_ss58_version(
				sp_core::crypto::Ss58AddressFormatRegistry::SelendraAccount.into(),
			);
		...
		"cardamom" | "cardamom-dev" => {
			sp_core::crypto::set_default_ss58_version(
				sp_core::crypto::Ss58AddressFormatRegistry::CardamomAccount.into(),
			);
			sub_tokens::dynamic::set_name("CDM");
			sub_tokens::dynamic::set_decimal_points(1_000_000_000_000);
			// safety: this program will always be single threaded, thus accessing global static is
			// safe.
			unsafe {
				RUNTIME = AnyRuntime::Cardamom;
			}
		},
		_ => {
...
mod tests {
	...
	fn any_runtime_works() {
		...
		let cardamom_version = any_runtime! { get_version::<Runtime>() };

		assert_eq!(selendra_version.spec_name, "selendra".into());
		assert_eq!(cardamom_version.spec_name, "cardamom".into());
	}
