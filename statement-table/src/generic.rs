...
pub type MisbehaviorFor<C> = Misbehavior<
    <C as Context>::Candidate,
    <C as Context>::Digest,
    <C as Context>::AuthorityId,
    <C as Context>::Signature,
>;
...
pub type ImportResult<C> =
    Result<Option<Summary<<C as Context>::Digest, <C as Context>::GroupId>>, MisbehaviorFor<C>>;
...
impl<C: Context> Table<C> {
    ...
    pub fn proposed_candidates(
        &self,
        context: &C,
    ) -> Vec<AttestedCandidate<C::GroupId, C::Candidate, C::AuthorityId, C::Signature>> {
        ...
        for candidate_data in self.candidate_votes.values() {
            ...
            if !candidate_data.can_be_included(threshold) {
                continue;
            }
        ...
        best_candidates
            ...
            .map(|&(candidate_data, threshold)| {
                candidate_data.attested(threshold).expect(
                    "candidate has been checked includable; \
    ...
    pub fn get_misbehavior(&self) -> &HashMap<C::AuthorityId, MisbehaviorFor<C>> {
    ...
    fn import_candidate(
        ...
        self.validity_vote(context, from, digest, ValidityVote::Issued(signature))
    }
    ...
    fn validity_vote(
                ...
                return if occ.get() != &vote {
                    Err(match (occ.get().clone(), vote) {
                        ...
                        | (ValidityVote::Invalid(bad), ValidityVote::Issued(iss)) => {
                            make_vdv(ValidityDoubleVote::IssuedAndInvalidity(
                                (votes.candidate.clone(), iss),
                                (digest, bad),
                            ))
                        }
        ...
        update_includable_count(
            &mut self.includable_count,
            &votes.group_id,
            was_includable,
            is_includable,
        );
