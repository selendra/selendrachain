...
fn purge_chain_works() {
	fn run_node_and_stop() -> tempfile::TempDir {
		...
		let mut cmd = Command::new(cargo_bin("selendra-collator"))
			.args(&["-d"])
			.arg(base_path.path())
			.args(&["--", "--dev"])
			.spawn()
	...
	// Check that both databases are deleted
	{
		...
		assert!(base_path.path().join("selendra/chains/dev/db/full").exists());

		let status = Command::new(cargo_bin("selendra-collator"))
			.args(&["purge-chain", "-d"])
			.arg(base_path.path())
			.arg("-y")
			.status()
		...
		assert!(!base_path.path().join("chains/local_testnet/db/full").exists());
		// assert!(base_path.path().join("selendra/chains/dev").exists());
		// assert!(!base_path.path().join("selendra/chains/dev/db").exists());
	}
}
