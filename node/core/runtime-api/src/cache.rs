...
use indracore_primitives::v1::{
    BlockNumber, CandidateCommitments, CandidateEvent, CommittedCandidateReceipt, CoreState,
...
pub(crate) struct RequestResultCache {
    ...
    historical_validation_code:
        MemoryLruCache<(Hash, ParaId, BlockNumber), ResidentSizeOf<Option<ValidationCode>>>,
...
impl RequestResultCache {
    ...
    pub(crate) fn historical_validation_code(
        &mut self,
        key: (Hash, ParaId, BlockNumber),
    ) -> Option<&Option<ValidationCode>> {
        self.historical_validation_code.get(&key).map(|v| &v.0)
    }

    pub(crate) fn cache_historical_validation_code(
        &mut self,
        key: (Hash, ParaId, BlockNumber),
        value: Option<ValidationCode>,
    ) {
        self.historical_validation_code
            .insert(key, ResidentSizeOf(value));
    }
