    ...
    Client::Api: ParachainHost<Block> + BabeApi<Block>,
{
    fn store_cache(&mut self, result: RequestResult) {
        ...
        match result {
            ...
            HistoricalValidationCode(relay_parent, para_id, n, code) => self
                .requests_cache
                .cache_historical_validation_code((relay_parent, para_id, n), code),
    ...
    Client::Api: ParachainHost<Block> + BabeApi<Block>,
{
    ...
    match request {
        ...
        Request::HistoricalValidationCode(para, at, sender) => query!(
            HistoricalValidationCode,
            historical_validation_code(para, at),
            sender
        ),
...
mod tests {
    ...
    use indracore_primitives::v1::{
        ...
        PersistedValidationData, SessionIndex, SessionInfo, ValidationCode, ValidatorId,
        ValidatorIndex,
    };
            ...
            ctx_handle
                .send(FromOverseer::Communication {
                    msg: RuntimeApiMessage::Request(
                        relay_parent,
                        Request::PersistedValidationData(
                            para_a,
        ...
        let test_task = async move {
            ...
            ctx_handle
                .send(FromOverseer::Communication {
                    msg: RuntimeApiMessage::Request(
                        relay_parent,
                        Request::SessionIndexForChild(tx),
                    ),
                })
                .await;
    ...
    fn requests_historical_code() {
        ...
        let runtime_api = Arc::new({
            ...
            runtime_api
                ...
                .insert(para_b, vec![(5, vec![7, 8, 9].into())]);
        ...
        let test_task = async move {
            {
                ...
                assert_eq!(
                    rx.await.unwrap().unwrap(),
                    Some(ValidationCode::from(vec![1, 2, 3]))
                );
            }

            {
                ...
                assert_eq!(
                    rx.await.unwrap().unwrap(),
                    Some(ValidationCode::from(vec![4, 5, 6]))
                );
            }
