...
fn delete_available_data(tx: &mut DBTransaction, hash: &CandidateHash) {
    let key = (AVAILABLE_PREFIX, hash).encode();
...
fn write_meta(tx: &mut DBTransaction, hash: &CandidateHash, meta: &CandidateMeta) {
    let key = (META_PREFIX, hash).encode();
...
fn delete_unfinalized_height(tx: &mut DBTransaction, block_number: BlockNumber) {
    ...
    loop {
        ...
        let batch_num = {
            let mut iter = subsystem
                .db
                .iter_with_prefix(columns::META, &start_prefix)
...
fn prune_all(db: &Arc<dyn KeyValueDB>, clock: &dyn Clock) -> Result<(), Error> {
    ...
    let mut tx = DBTransaction::new();
    let iter = db
        .iter_with_prefix(columns::META, &range_start[..])
