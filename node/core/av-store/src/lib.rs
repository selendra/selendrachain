...
use indracore_primitives::v1::{
    AvailableData, BlockNumber, CandidateEvent, CandidateHash, ErasureChunk, Hash, ValidatorIndex,
};
...
impl PruningDelay {
    ...
    fn into_the_future(duration: Duration) -> Result<Self, Error> {
        Ok(Self::In(
            SystemTime::now().duration_since(UNIX_EPOCH)? + duration,
        ))
    }
...
impl PartialEq for ChunkPruningRecord {
    fn eq(&self, other: &Self) -> bool {
        self.candidate_hash == other.candidate_hash && self.chunk_index == other.chunk_index
    }
}
...
impl AvailabilityStoreSubsystem {
    ...
    fn prune_povs(&self) -> Result<(), Error> {
        ...
        let outdated_records_count = pov_pruning.iter().take_while(|r| r.prune_at <= now).count();
    ...
    fn prune_chunks(&self) -> Result<(), Error> {
        ...
        let outdated_records_count = chunk_pruning
            .iter()
    ...
    fn maybe_prune_povs(&self) -> Result<impl Future<Output = ()>, Error> {
        let future = match get_next_pov_pruning_time(&self.inner) {
            Some(pruning) => Either::Left(Delay::new(pruning.should_fire_in()?)),
...
fn put_pov_pruning(
    ...
    tx.put_vec(columns::META, &POV_PRUNING_KEY, pov_pruning.encode());
    ...
    match pov_pruning.get(0) {
        ...
        Some(PoVPruningRecord {
            prune_at: PruningDelay::In(prune_at),
            ..
        }) => {
            tx.put_vec(
                columns::META,
...
fn put_chunk_pruning(
    ...
    tx.put_vec(columns::META, &CHUNK_PRUNING_KEY, chunk_pruning.encode());

    match chunk_pruning.get(0) {
        Some(ChunkPruningRecord {
            prune_at: PruningDelay::In(prune_at),
            ..
        }) => {
            tx.put_vec(
    ...
    Context: SubsystemContext<Message = AvailabilityStoreMessage>,
{
    ...
    ctx.send_message(AllMessages::ChainApi(ChainApiMessage::BlockNumber(
        block_hash, tx,
    )))
    .await;
...
impl Metrics {
    ...
    fn time_prune_povs(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.prune_povs.start_timer())
    }
    ...
    fn time_prune_chunks(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.prune_chunks.start_timer())
    }
