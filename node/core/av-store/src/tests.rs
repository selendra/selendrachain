...
fn forkfullness_works() {
    ...
    test_harness(
        ...
        |test_harness| async move {
            ...
            assert!(
                query_all_chunks(&mut virtual_overseer, candidate_2_hash, n_validators, false)
                    .await,
            );
            ...
            assert!(
                query_all_chunks(&mut virtual_overseer, candidate_1_hash, n_validators, false)
                    .await,
            );
...
async fn query_all_chunks(
    ...
    for i in 0..n_validators {
        if query_chunk(virtual_overseer, candidate_hash, i)
            .await
            .is_some()
            != expect_present
        {
