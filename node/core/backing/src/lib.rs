...
impl CandidateBackingJob {
                    ...
                    Ok((candidate, commitments, _)) => {
                            ...
                            && !self.issued_statements.contains(&candidate_hash)
                        {
                                ...
                                .await?
                            {
                                sender
                                    .send_message(
                                        CandidateSelectionMessage::Seconded(self.parent, stmt)
                                            .into(),
                                    )
                                    .await;
                            }
            ...
            ValidatedCandidateCommand::Attest(res) => {
                ...
                if !self.issued_statements.contains(&candidate_hash) {
                    if res.is_ok() {
                        ...
                        self.sign_import_and_distribute_statement(sender, statement, &parent_span)
                            .await?;
                    }
                    self.issued_statements.insert(candidate_hash);
                }
    ...
    async fn sign_import_and_distribute_statement(
        ...
        if let Some(signed_statement) = self.sign_statement(statement).await {
            self.import_statement(sender, &signed_statement, parent_span)
                .await?;
    ...
    async fn process_msg(
        ...
        match msg {
            ...
            CandidateBackingMessage::Statement(_relay_parent, statement) => {
                ...
                match self
                    .maybe_validate_and_import(&span, &root_span, sender, statement)
                    .await
                {
    ...
    async fn maybe_validate_and_import(
        ...
        if let Some(summary) = self
            .import_statement(sender, &statement, parent_span)
            .await?
        {
