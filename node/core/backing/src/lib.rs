...
async fn request_pov(
    ...
    tx_from
        .send(FromJobCommand::SendMessage(
            AllMessages::AvailabilityDistribution(AvailabilityDistributionMessage::FetchPoV {
                relay_parent,
                from_validator,
                candidate_hash,
                pov_hash,
...
async fn validate_and_make_available(
    ...
    let v = {
        ...
        request_candidate_validation(&mut tx_from, candidate.descriptor.clone(), pov.clone())
            .await?
    };
...
impl CandidateBackingJob {
    ...
    async fn issue_new_misbehaviors(&mut self) -> Result<(), Error> {
        ...
        for (validator_id, report) in misbehaviors {
            self.send_to_provisioner(ProvisionerMessage::ProvisionableData(
                self.parent,
        ...
        match msg {
            CandidateBackingMessage::Second(relay_parent, candidate, pov) => {
                ...
                if self.seconded.is_none() {
                    ...
                    if !self.issued_statements.contains(&candidate_hash) {
                        let pov = Arc::new(pov);
                        self.validate_and_second(&span, &root_span, &candidate, pov)
                            .await?;
                    }
                }
            }
            CandidateBackingMessage::Statement(_relay_parent, statement) => {
                ...
                match self
                    .maybe_validate_and_import(&span, &root_span, statement)
                    .await
                {
    ...
    async fn maybe_validate_and_import(
        ...
        if let Some(summary) = self.import_statement(&statement, parent_span).await? {
            ...
            self.kick_off_validation_work(attesting, span).await?;
        }
        Ok(())
    }
...
mod tests {
    ...
    struct TestHarness {
        virtual_overseer: indracore_node_subsystem_test_helpers::TestSubsystemContextHandle<
    ...
    fn test_harness<T: Future<Output = ()>>(
        ...
        let (context, virtual_overseer) =
            indracore_node_subsystem_test_helpers::make_subsystem_context(pool.clone());

        let subsystem =
            CandidateBackingSubsystem::run(context, keystore, Metrics(None), pool.clone());
    ...
    async fn test_startup(
        virtual_overseer: &mut indracore_node_subsystem_test_helpers::TestSubsystemContextHandle<
            CandidateBackingMessage,
        >,
