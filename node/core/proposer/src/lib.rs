...
where
    ...
    Client: 'static
        ...
        + Send
        + Sync,
    Client::Api: BlockBuilderApi<Block> + ApiExt<Block>,
    Backend:
    ...
    sp_api::StateBackendFor<Client, Block>: sp_api::StateBackend<HashFor<Block>> + Send,
{
    ...
    async fn get_provisioner_data(&self) -> Result<ProvisionerInherentData, Error> {
        ...
        let pid = async {
            ...
            overseer
                .wait_for_activation(parent_header_hash, sender)
                .await;
            receiver
                .await
    ...
    sp_api::StateBackendFor<Client, Block>: sp_api::StateBackend<HashFor<Block>> + Send,
{
    ...
    type Proposal = Pin<
        Box<
            dyn Future<
                    Output = Result<Proposal<Block, sp_api::TransactionFor<Client, Block>>, Error>,
                > + Send,
        >,
    >;
    type Error = Error;
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::ClosedChannelAwaitingActivation => {
                write!(f, "closed channel from overseer when awaiting activation")
            }
            Self::ClosedChannelAwaitingInherentData => write!(
                f,
                "closed channel from provisioner when awaiting inherent data"
            ),
