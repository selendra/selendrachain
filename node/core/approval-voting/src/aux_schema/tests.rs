...
impl TestStore {
    fn write_stored_blocks(&self, range: StoredBlockRange) {
        self.inner
            .borrow_mut()
            .insert(STORED_BLOCKS_KEY.to_vec(), range.encode());
    }

    fn write_blocks_at_height(&self, height: BlockNumber, blocks: &[Hash]) {
        self.inner
            .borrow_mut()
            .insert(blocks_at_height_key(height).to_vec(), blocks.encode());
    }

    fn write_block_entry(&self, block_hash: &Hash, entry: &BlockEntry) {
        self.inner
            .borrow_mut()
            .insert(block_entry_key(block_hash).to_vec(), entry.encode());
    }

    fn write_candidate_entry(&self, candidate_hash: &CandidateHash, entry: &CandidateEntry) {
        self.inner
            .borrow_mut()
            .insert(candidate_entry_key(candidate_hash).to_vec(), entry.encode());
    }
}
...
fn make_block_entry(block_hash: Hash, candidates: Vec<(CoreIndex, CandidateHash)>) -> BlockEntry {
    BlockEntry {
        block_hash,
        session: 1,
        slot: 1,
...
fn read_write() {
    ...
    let block_entry = make_block_entry(hash_a, vec![(CoreIndex(0), candidate_hash)]);
    ...
    assert_eq!(
        load_block_entry(&store, &hash_a).unwrap(),
        Some(block_entry)
    );
    assert_eq!(
        load_candidate_entry(&store, &candidate_hash).unwrap(),
    ...
    assert!(load_candidate_entry(&store, &candidate_hash)
        .unwrap()
        .is_none());
}
...
fn add_block_entry_works() {
    ...
    let block_entry_a = make_block_entry(block_hash_a, vec![(CoreIndex(0), candidate_hash_a)]);
    ...
    assert_eq!(
        load_block_entry(&store, &block_hash_a).unwrap(),
        Some(block_entry_a)
    );
    assert_eq!(
        load_block_entry(&store, &block_hash_b).unwrap(),
        Some(block_entry_b)
    );

    let candidate_entry_a = load_candidate_entry(&store, &candidate_hash_a)
        .unwrap()
        .unwrap();
    assert_eq!(
        candidate_entry_a
    ...
    let candidate_entry_b = load_candidate_entry(&store, &candidate_hash_b)
        .unwrap()
        .unwrap();
    assert_eq!(
        candidate_entry_b
...
fn add_block_entry_adds_child() {
    ...
    let mut block_entry_a = make_block_entry(block_hash_a, Vec::new());

    let block_entry_b = make_block_entry(block_hash_b, Vec::new());
...
fn canonicalize_works() {
    ...
    let check_candidates_in_store = |expected: Vec<(CandidateHash, Option<Vec<_>>)>| {
        for (c_hash, in_blocks) in expected {
            let (entry, in_blocks) = match in_blocks {
                ...
                Some(i) => (load_candidate_entry(&store, &c_hash).unwrap().unwrap(), i),
            };
    ...
    let check_blocks_in_store = |expected: Vec<(Hash, Option<Vec<_>>)>| {
        for (hash, with_candidates) in expected {
            let (entry, with_candidates) = match with_candidates {
                ...
                Some(i) => (load_block_entry(&store, &hash).unwrap().unwrap(), i),
            };
    ...
    assert_eq!(
        load_stored_blocks(&store).unwrap().unwrap(),
