...
fn add_block_entry_works() {
    ...
    add_block_entry(&store, block_entry_a.clone(), n_validators, |h| {
    ...
    add_block_entry(&store, block_entry_b.clone(), n_validators, |h| {
        new_candidate_info.get(h).map(|x| x.clone())
    })
    .unwrap();
...
fn add_block_entry_adds_child() {
    ...
    add_block_entry(&store, block_entry_a.clone(), n_validators, |_| None).unwrap();

    add_block_entry(&store, block_entry_b.clone(), n_validators, |_| None).unwrap();
...
fn canonicalize_works() {
    ...
    for block_entry in blocks {
        add_block_entry(&store, block_entry, n_validators, |h| {
...
fn force_approve_works() {
    ...
    assert!(load_block_entry(&store, &block_hash_a)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .all());
    assert!(load_block_entry(&store, &block_hash_b)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .all());
    assert!(load_block_entry(&store, &block_hash_c)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .not_any());
    assert!(load_block_entry(&store, &block_hash_d)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .not_any());
