...
use indracore_primitives::v1::{
    ...
    GroupIndex, Hash, Header, SessionIndex, SessionInfo,
};
use kvdb::KeyValueDB;
...
mod tests {
    ...
    fn insta_approval_works() {
        ...
        let test_fut = {
            Box::pin(async move {
                let result = handle_new_head(&mut ctx, &mut state, &db_writer, hash, &Some(1))
                    .await
                    .unwrap();
    ...
    fn any_session_unavailable_for_caching_means_no_change() {
        ...
        let test_fut = {
            let header = header.clone();
            Box::pin(async move {
                let res = cache_session_info_for_head(&mut ctx, &mut window, hash, &header).await;
