...
use indracore_subsystem::{
    ...
    messages::{
        ...
        AssignmentCheckResult, AvailabilityRecoveryMessage, CandidateValidationMessage,
        ChainApiMessage, RuntimeApiMessage, RuntimeApiRequest, ValidationFailed,
    },
...
use std::ops::{Bound as RangeBound, RangeBounds};
use std::sync::Arc;
...
impl Wakeups {
    ...
    fn drain<'a, R: RangeBounds<Tick>>(
        ...
        prev.into_iter()
            ...
            .inspect(move |&(ref b, ref c)| {
                let _ = reverse.remove(&(*b, *c));
            })
    }
}
    ...
    C: SubsystemContext<Message = ApprovalVotingMessage>,
{
    ...
    loop {
        let wait_til_next_tick = match wakeups.first() {
            ...
            Some(tick) => future::Either::Right(state.clock.wait(tick).map(move |()| tick)),
        };
        ...
        if handle_actions(&mut ctx, &mut wakeups, db_writer, &background_tx, actions).await? {
            break;
        }
    }
...
fn check_and_import_approval<T>(
    ...
    let approval_payload =
        approval_signing_payload(ApprovalVote(approved_candidate_hash), block_entry.session());
    ...
    if candidate_entry
        .approval_entry(&approval.block_hash)
...
fn check_and_apply_full_approval(
    ...
    let mut actions = Vec::new();
    for (block_hash, approval_entry) in candidate_entry
        .iter_approval_entries()
        .into_iter()
...
async fn launch_approval(
    ...
    let in_context_number = match context_num_rx
        .await?
...
fn sign_approval(
    ...
    let payload = approval_signing_payload(ApprovalVote(candidate_hash), session_index);
