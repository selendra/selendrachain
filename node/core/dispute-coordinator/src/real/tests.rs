...
fn make_keystore(accounts: &[Sr25519Keyring]) -> LocalKeystore {
	...
	for s in accounts.iter().copied().map(|k| k.to_seed()) {
		store
			.sr25519_generate_new(selendra_primitives::v1::PARACHAIN_KEY_TYPE_ID, Some(s.as_str()))
			.unwrap();
	}
...
struct TestState {
	...
	polkadot-v0.9.13_keystore: Arc<sc_keystore::LocalKeystore>,
...
impl Default for TestState {
	fn default() -> TestState {
		...
		let polkadot-v0.9.13_keystore = make_keystore(&validators).into();
...
impl TestState {
	...
	async fn issue_statement_with_index(
		...
		let keystore = self.polkadot-v0.9.13_keystore.clone() as SyncCryptoStorePtr;
	...
	test_harness(|mut test_state, mut virtual_overseer| {
		Box::pin(async move {
			...
			let supermajority_threshold =
				selendra_primitives::v1::supermajority_threshold(test_state.validators.len());
