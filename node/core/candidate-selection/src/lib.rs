        ...
        loop {
            match self.receiver.next().await {
                Some(CandidateSelectionMessage::Collation(relay_parent, para_id, collator_id)) => {
                    ...
                    self.handle_collation(sender, relay_parent, para_id, collator_id)
                        .await;
                }
    ...
    async fn handle_collation(
        ...
        if self.seconded_candidate.is_none() {
            let (candidate_receipt, pov) =
                match get_collation(relay_parent, para_id, collator_id.clone(), sender).await {
            ...
            second_candidate(relay_parent, candidate_receipt, pov, sender, &self.metrics).await;
...
async fn get_collation(
    ...
    sender
        .send_message(
            CollatorProtocolMessage::FetchCollation(relay_parent, collator_id, para_id, tx).into(),
        )
        .await;
...
async fn second_candidate(
    ...
    sender
        .send_message(CandidateBackingMessage::Second(relay_parent, candidate_receipt, pov).into())
        .await;

    metrics.on_second();
}

async fn forward_invalidity_note(received_from: &CollatorId, sender: &mut impl SubsystemSender) {
    sender
...
mod tests {
    ...
    fn test_harness<Preconditions, TestBuilder, Test, Postconditions>(
        ...
        TestBuilder: FnOnce(
            mpsc::Sender<CandidateSelectionMessage>,
        ...
        Postconditions: FnOnce(CandidateSelectionJob, Result<(), Error>),
    {
        ...
        let (mut from_job_tx, from_job_rx) =
        ...
        let (_, (job, job_result)) =
            futures::executor::block_on(future::join(test(to_job_tx, from_job_rx), async move {
