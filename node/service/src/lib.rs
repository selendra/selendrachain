...
pub use {
	...
	selendra_primitives::v2::ParachainHost,
...
pub use selendra_client::CardamomExecutorDispatch;

#[cfg(feature = "selendra-native")]
pub use selendra_client::SelendraExecutorDispatch;

pub use chain_spec::{CardamomChainSpec, SelendraChainSpec};
...
pub enum Error {
	...
	#[cfg(feature = "full-node")]
	#[error("Expected at least one of polkadot, cardamom, runtime feature")]
	NoRuntime,
}
	...
	ChainSelection: 'static + SelectChain<Block>,
{
	...
	let grandpa_hard_forks = if config.chain_spec.is_cardamom() {
		grandpa_support::cardamom_hard_forks()
	} else {
		Vec::new()
	};
	...
	OverseerGenerator: OverseerGen,
{
	...
	let disputes_enabled = chain_spec.is_cardamom();
	...
	let _beefy_protocol_name = beefy_gadget::protocol_standard_name(
		&client.block_hash(0).ok().flatten().expect("Genesis block exists; qed"),
		&config.chain_spec,
	);
...
/// Build a full node.
///
/// The actual "flavor", aka if it will use `Selendra`,` or `Cardamom` is determined based on
