    ...
    B: sp_blockchain::HeaderBackend<IndracoreBlock>,
{
    fn restrict_vote(
        ...
        let aux = || {
            let find_target = |target_number: BlockNumber, current_header: &IndracoreHeader| {
            ...
            let target_number = std::cmp::max(
                current_target
                    .number()
                    .saturating_sub(DIAGNOSTIC_GRANDPA_DELAY),
                base.number().clone(),
            );
        ...
        Box::pin(async move {
            ...
            let approval_checking_subsystem_lag = approval_checking_subsystem_vote
                .map_or(current_number - base_number, |(_h, n)| current_number - n);
...
mod tests {
    ...
    use indracore_test_client::{
        ClientBlockImportExt, DefaultTestClientBuilderExt, InitIndracoreBlockBuilder,
