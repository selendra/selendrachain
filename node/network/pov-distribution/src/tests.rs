...
use indracore_node_network_protocol::{our_view, view};
use indracore_node_subsystem_test_helpers as test_helpers;
use indracore_node_subsystem_util::TimeoutExt;
use indracore_primitives::v1::{
    AuthorityDiscoveryId, BlockData, CoreState, GroupRotationInfo, Id as ParaId, ScheduledCore,
...
fn make_peer_state(awaited: Vec<(Hash, Vec<Hash>)>) -> PeerState {
    PeerState {
        awaited: awaited
            .into_iter()
            .map(|(rp, h)| (rp, h.into_iter().collect()))
            .collect(),
    }
}
...
fn test_harness<T: Future<Output = ()>>(state: State, test: impl FnOnce(TestHarness) -> T) {
    let _ = env_logger::builder()
        .is_test(true)
        .filter(Some("indracore_pov_distribution"), log::LevelFilter::Trace)
...
async fn overseer_send(overseer: &mut VirtualOverseer, msg: PoVDistributionMessage) {
...
fn ask_validators_for_povs() {
    ...
    test_harness(State::default(), |test_harness| async move {
        ...
        for i in vec![2, 0, 4] {
            overseer_send(
                &mut virtual_overseer,
                PoVDistributionMessage::NetworkBridgeUpdateV1(NetworkBridgeEvent::PeerViewChange(
        ...
        overseer_send(
            &mut virtual_overseer,
            PoVDistributionMessage::NetworkBridgeUpdateV1(NetworkBridgeEvent::PeerMessage(
...
fn distributes_to_those_awaiting_and_completes_local() {
    ...
    let state = State {
        ...
        peer_state: {
            ...
            s.insert(peer_b.clone(), make_peer_state(vec![(hash_a, vec![])]));
    ...
    test_harness(state, |test_harness| async move {
        ...
        overseer_send(
            &mut virtual_overseer,
            PoVDistributionMessage::DistributePoV(hash_a, descriptor, Arc::new(pov.clone())),
        )
        .await;
...
fn we_inform_peers_with_same_view_we_are_awaiting() {
    ...
    let mut state = State {
        ...
        peer_state: {
            ...
            s.insert(peer_a.clone(), make_peer_state(vec![(hash_a, vec![])]));

            // peer B doesn't have hash_a in its view.
            s.insert(peer_b.clone(), make_peer_state(vec![(hash_b, vec![])]));
    ...
    executor::block_on(async move {
        let handle_future = handle_fetch(&mut state, &mut ctx, hash_a, descriptor, pov_send);
...
fn peer_view_change_leads_to_us_informing() {
    ...
    let mut state = State {
        ...
        peer_state: {
            ...
            s.insert(peer_a.clone(), make_peer_state(vec![(hash_b, vec![])]));
...
fn peer_punished_for_sending_bad_pov() {
    ...
    executor::block_on(async move {
        ...
        assert_eq!(
            state.relay_parent_state[&hash_a].fetching[&pov_hash].len(),
            1
        );
...
fn peer_reported_for_awaiting_too_much() {
    ...
    executor::block_on(async move {
        ...
        assert_eq!(
            state.peer_state[&peer_a].awaited[&hash_a].len(),
            max_plausibly_awaited
        );
