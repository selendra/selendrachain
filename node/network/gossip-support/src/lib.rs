...
use indracore_node_subsystem::{
    messages::GossipSupportMessage, ActiveLeavesUpdate, FromOverseer, OverseerSignal,
    SpawnedSubsystem, Subsystem, SubsystemContext,
};
use indracore_node_subsystem_util::{
    self as util,
    validator_discovery::{self, ConnectionRequest},
};
...
async fn determine_relevant_validators(
    ...
    let validators = util::request_validators_ctx(relay_parent, ctx)
        .await?
        .await??;
    Ok(validators)
}
...
impl State {
    ...
    async fn handle_active_leaves(
        ...
        for leaf in leaves {
            let current_index = util::request_session_index_for_child_ctx(leaf, ctx)
                .await?
                .await??;
            ...
            if let Some((new_session, relay_parent)) = maybe_new_session {
                ...
                let validators =
                    determine_relevant_validators(ctx, relay_parent, new_session).await?;
                let validators = choose_random_subset(validators);
                tracing::debug!(
                    target: LOG_TARGET,
                    "Issuing a connection request to {:?}",
                    validators
                );
