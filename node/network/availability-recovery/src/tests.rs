...
fn test_harness_fast_path<T: Future<Output = ()>>(test: impl FnOnce(TestHarness) -> T) {
...
fn test_harness_chunks_only<T: Future<Output = ()>>(test: impl FnOnce(TestHarness) -> T) {
    let _ = env_logger::builder()
        .is_test(true)
        .filter(
            Some("indracore_availability_recovery"),
...
impl TestState {
    ...
    async fn test_connect_to_all_validators(&self, virtual_overseer: &mut VirtualOverseer) {
        self.test_connect_to_validators(virtual_overseer, self.validator_public.len())
            .await;
    }

    async fn test_connect_to_validators(&self, virtual_overseer: &mut VirtualOverseer, n: usize) {
...
fn availability_is_recovered_from_chunks_if_no_group_provided() {
    ...
    test_harness_fast_path(|test_harness| async move {
        ...
        test_state
            .test_connect_to_all_validators(&mut virtual_overseer)
            .await;

        let candidate_hash = test_state.candidate.hash();

        test_state
            .test_chunk_requests(candidate_hash, &mut virtual_overseer)
            .await;
...
fn bad_merkle_path_leads_to_recovery_error() {
    ...
    test_harness_fast_path(|test_harness| async move {
        ...
        test_state
            .test_faulty_chunk_requests(candidate_hash, &mut virtual_overseer, &faulty)
            .await;
...
fn fast_path_backing_group_recovers() {
    ...
    test_harness_fast_path(|test_harness| async move {
        ...
        let mut who_has: Vec<_> = (0..test_state.validators.len())
            .map(|_| HasAvailableData::No)
            .collect();
...
fn wrong_data_from_fast_path_peer_leads_to_punishment() {
    ...
    test_harness_fast_path(|test_harness| async move {
        ...
        a.pov = Arc::new(PoV {
            block_data: BlockData(vec![69; 420]),
        });

        let who_has: Vec<_> = (0..test_state.validators.len())
            .map(|_| HasAvailableData::Other(a.clone()))
            .collect();
...
fn no_answers_in_fast_path_causes_chunk_requests() {
    ...
    test_harness_fast_path(|test_harness| async move {
        ...
        let mut who_has: Vec<_> = (0..test_state.validators.len())
            .map(|_| HasAvailableData::Timeout)
            .collect();
