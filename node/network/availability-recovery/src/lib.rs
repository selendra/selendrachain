...
const LOG_TARGET: &str = "parachain::availability_recovery";
    ...
    async fn launch_parallel_requests(
        ...
        while self.requesting_chunks.len() < N_PARALLEL {
            if let Some(validator_index) = self.shuffling.pop() {
                ...
                self.requesting_chunks
                    .push(rx.timeout(CHUNK_REQUEST_TIMEOUT));
            } else {
                break;
            }
        }
    ...
    async fn wait_for_chunks(
        ...
        while let Some(request_result) = self.requesting_chunks.next().await {
            match request_result {
                ...
                None => {
                    tracing::debug!(target: LOG_TARGET, "A chunk request has timed out",);
                }
            }
        }
        ...
        loop {
            ...
            match self.phase {
                InteractionPhase::RequestFromBackers(ref mut from_backers) => {
                    if from_backers
                        .run(&self.params, &mut self.to_state)
                        .await
    ...
    match from_interaction {
        ...
        FromInteraction::MakeFullDataRequest(id, candidate_hash, validator_index, response) => {
            ...
            state
                ...
                .entry(id)
                .or_default()
                .push(Awaited::FullData(AwaitedData {
                    validator_index,
                    candidate_hash,
                    token,
                    response,
        ...
        NetworkBridgeEvent::PeerMessage(peer, message) => {
            match message {
                    ...
                    match state.live_requests.remove(&request_id) {
                        ...
                        Some((peer_id, Awaited::Chunk(awaited_chunk))) if peer_id == peer => {
                            ...
                            if let Some(chunk) = chunk {
                                if awaited_chunk
                                    .response
                                    .send((peer_id, awaited_chunk.validator_index, chunk))
                                    .is_err()
                                {
                                    tracing::debug!(
                                        target: LOG_TARGET,
                ...
                protocol_v1::AvailabilityRecoveryMessage::RequestFullData(
                    ...
                    let wire_message =
                        protocol_v1::AvailabilityRecoveryMessage::FullData(request_id, full_data);
                    ...
                    match state.live_requests.remove(&request_id) {
                        ...
                        Some((peer_id, Awaited::FullData(awaited))) if peer_id == peer => {
                            ...
                            if let Some(data) = data {
                                if awaited
                                    .response
                                    .send((peer_id, awaited.validator_index, data))
                                    .is_err()
                                {
                                    tracing::debug!(
                                        target: LOG_TARGET,
