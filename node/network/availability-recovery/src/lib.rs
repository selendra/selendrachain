...
use indracore_primitives::v1::{
    ...
    ErasureChunk, Hash, HashT, SessionIndex, SessionInfo, ValidatorId, ValidatorIndex,
};
    ...
    async fn launch_parallel_requests(&mut self) -> error::Result<()> {
        while self.requesting_chunks.len() < N_PARALLEL {
            if let Some(validator_index) = self.shuffling.pop() {
                ...
                self.to_state
                    .send(FromInteraction::MakeRequest(
            ...
            match request_result {
                Some(Ok(Ok((peer_id, chunk)))) => {
                    ...
                    if let Ok(anticipated_hash) =
                        branch_hash(&self.erasure_root, &chunk.proof, chunk.index as usize)
                    {
...
async fn issue_chunk_request(
    ...
    state
        .live_chunk_requests
        .insert(request_id, (peer_id, awaited_chunk));
...
fn cleanup_awaited_chunks(state: &mut State) {
    ...
    state
        .live_chunk_requests
        .retain(|_, v| !v.1.response.is_canceled());
}
