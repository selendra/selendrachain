...
struct RequestChunksPhase {
    ...
    requesting_chunks:
        FuturesUnordered<BoxFuture<'static, Result<Option<ErasureChunk>, RequestError>>>,
}
...
impl RequestFromBackersPhase {
    ...
    async fn run(
        ...
        loop {
            ...
            to_state
                .send(FromInteraction::NetworkRequest(
...
impl RequestChunksPhase {
    ...
    async fn launch_parallel_requests(
        ...
        while self.requesting_chunks.len() < max_requests {
            if let Some(validator_index) = self.shuffling.pop() {
                ...
                to_state
                    .send(FromInteraction::NetworkRequest(Requests::ChunkFetching(
                        req,
                    )))
                    .await?;
    ...
    async fn wait_for_chunks(&mut self, params: &InteractionParams) -> Result<(), mpsc::SendError> {
...
async fn handle_from_interaction(
    ...
    match from_interaction {
        ...
        FromInteraction::NetworkRequest(request) => {
            ctx.send_message(
                NetworkBridgeMessage::SendRequests(vec![request], IfDisconnected::TryConnect)
                    .into(),
            )
            .await;
        }
