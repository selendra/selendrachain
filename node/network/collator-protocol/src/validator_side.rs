...
use futures::{
    channel::oneshot,
    future::{BoxFuture, Either, Fuse, FusedFuture},
    FutureExt,
};
use futures_timer::Delay;

use indracore_node_network_protocol::request_response as req_res;
use indracore_node_network_protocol::request_response::v1::{
    CollationFetchingRequest, CollationFetchingResponse,
};
...
async fn report_collator<Context>(ctx: &mut Context, state: &mut State, id: CollatorId)
where
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
...
#[tracing::instrument(level = "trace", skip(ctx, state), fields(subsystem = LOG_TARGET))]
async fn note_good_collation<Context>(ctx: &mut Context, state: &mut State, id: CollatorId)
where
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
...
async fn handle_peer_view_change(state: &mut State, peer_id: PeerId, view: View) -> Result<()> {
    ...
    for removed in removed.into_iter() {
        state
            ...
            .retain(|k, _| k.0 != removed || k.2 != peer_id);
    }
    ...
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
    ctx.send_message(AllMessages::CandidateSelection(
        CandidateSelectionMessage::Collation(relay_parent, para_id, collator),
    ))
    .await;
}
    ...
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
    ...
    loop {
        let res = {
            let s =
                futures::future::select(ctx.recv().fuse(), next_inactivity_stream.next().fuse());
