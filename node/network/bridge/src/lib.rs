...
async fn update_our_view(
    ...
    *local_view = new_view.clone();
    ...
    dispatch_validation_event_to_all(NetworkBridgeEvent::OurViewChange(our_view.clone()), ctx)
        .await;
        ...
        outgoing_messages.push(match message {
            WireMessage::ViewUpdate(new_view) => {
                    ...
                    || new_view.finalized_number < peer_data.view.finalized_number
                {
                    ...
                    net.report_peer(peer.clone(), EMPTY_VIEW_COST).await?;
...
mod tests {
        ...
        test_harness(|test_harness| async move {
            ...
            assert_network_actions_contains(
                ...
                &NetworkAction::WriteNotification(
                    peer.clone(),
                    PeerSet::Collation,
                    WireMessage::<protocol_v1::CollationProtocol>::ViewUpdate(view.clone())
                        .encode(),
                ),
            );
        });
    ...
    fn do_not_send_view_update_when_only_finalized_block_changed() {
        test_harness(|test_harness| async move {
            ...
            let actions = network_handle.next_network_actions(4).await;
            let wire_message = WireMessage::<protocol_v1::ValidationProtocol>::ViewUpdate(View {
                heads: vec![hash_a],
                finalized_number: 5,
            })
            .encode();
    ...
    fn peer_messages_sent_via_overseer() {
        test_harness(|test_harness| async move {
            ...
            let pov_distribution_message =
                protocol_v1::PoVDistributionMessage::Awaiting([0; 32].into(), vec![[1; 32].into()]);

            let message =
                protocol_v1::ValidationProtocol::PoVDistribution(pov_distribution_message.clone());
        ...
        test_harness(|test_harness| async move {
            ...
            network_handle
                .peer_message(
                    peer.clone(),
                    PeerSet::Validation,
                    WireMessage::<protocol_v1::ValidationProtocol>::ViewUpdate(view_a.clone())
                        .encode(),
                )
                .await;
            ...
            network_handle
                .peer_message(
                    peer.clone(),
                    PeerSet::Collation,
                    WireMessage::<protocol_v1::CollationProtocol>::ViewUpdate(view_b.clone())
                        .encode(),
                )
                .await;
        ...
        test_harness(|test_harness| async move {
            ...
            network_handle
                .peer_message(
                    peer_a.clone(),
                    PeerSet::Validation,
                    WireMessage::<protocol_v1::ValidationProtocol>::ViewUpdate(View {
                        heads: vec![Hash::repeat_byte(0x01)],
                        finalized_number: 1,
                    })
                    .encode(),
                )
