...
impl<N: Network, AD: AuthorityDiscovery> Service<N, AD> {
    ...
    async fn find_connected_validators(
        ...
        for id in validator_ids {
            ...
            if let Some(pid) = state.connected_peers.iter().find_map(|(pid, ids)| {
                if ids.contains(&id) {
                    Some(pid)
                } else {
                    None
    ...
    pub async fn on_request(
        ...
        let already_connected = self
            .find_connected_validators(&validator_ids, peer_set, &mut authority_discovery_service)
            .await;
        ...
        validator_ids
            .iter()
            .for_each(|id| *state.requested_validators.entry(id.clone()).or_default() += 1);
        ...
        if let Err(e) = network_service
            .add_peers_to_reserved_set(peer_set.into_protocol_name(), multiaddr_to_add.clone())
            .await
        {
...
mod tests {
    ...
    fn requests_are_fulfilled_immediately_for_already_connected_peers() {
        ...
        futures::executor::block_on(async move {
            ...
            service
                .on_peer_connected(peer_ids[0].clone(), PeerSet::Validation, &mut ads)
                .await;

            let _ = service
                .on_request(req1, PeerSet::Validation, sender, ns, ads)
                .await;
    ...
    fn requests_are_fulfilled_on_peer_connection() {
        ...
        futures::executor::block_on(async move {
            ...
            service
                .on_peer_connected(peer_ids[1].clone(), PeerSet::Validation, &mut ads)
                .await;
    ...
    fn handle_validator_connect_without_authority_discovery_knowing_it() {
        ...
        futures::executor::block_on(async move {
            ...
            service
                .on_peer_connected(validator_peer_id.clone(), PeerSet::Validation, &mut ads)
                .await;
