...
use super::{error::recv_runtime, Error};
...
impl Runtime {
    ...
    async fn get_validator_info(&self, session_info: &SessionInfo) -> Result<ValidatorInfo, Error> {
    ...
    async fn get_our_index(&self, validators: &[ValidatorId]) -> Option<ValidatorIndex> {
        for (i, v) in validators.iter().enumerate() {
            if CryptoStore::has_keys(&*self.keystore, &[(v.to_raw_vec(), ValidatorId::ID)]).await {
