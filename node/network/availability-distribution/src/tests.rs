...
use indracore_primitives::v1::{
    AvailableData, BlockData, CandidateCommitments, CandidateDescriptor, CommittedCandidateReceipt,
    GroupIndex, GroupRotationInfo, HeadData, Id as ParaId, OccupiedCore, PersistedValidationData,
    PoV, ScheduledCore,
};
...
fn make_erasure_chunks(
    peristed: PersistedValidationData,
...
fn helper_integrity() {
    ...
    let message = make_valid_availability_gossip(&test_state, 0, 2);
...
async fn change_our_view(
    ...
    for _ in 0..data_availability.len() {
        ...
        if let Some((pov, persisted)) = chunk_data_per_candidate.get(&candidate_hash) {
            let chunks =
                make_erasure_chunks(persisted.clone(), validator_public.len(), pov.clone());
...
async fn peer_send_message(
    ...
    overseer_send(
        virtual_overseer,
        NetworkBridgeEvent::PeerMessage(peer.clone(), chunk_protocol_message(message)),
    )
    .await;
...
fn reputation_verification() {
    ...
    test_harness(keystore, move |test_harness| async move {
        ...
        let valid = make_valid_availability_gossip(&test_state, 0, 2);
...

        {
            ...
            let send_peers = chunks
                .iter()
                .map(|_| vec![peer_a.clone()])
                .collect::<Vec<_>>();
...
fn not_a_live_candidate_is_detected() {
    ...
    test_harness(keystore, move |test_harness| async move {
        ...
        let valid = make_valid_availability_gossip(&test_state, 1, 1);

        peer_send_message(
            &mut virtual_overseer,
            peer_a.clone(),
            valid.clone(),
            COST_NOT_A_LIVE_CANDIDATE,
        )
        .await;
    });
}
...
fn peer_change_view_before_us() {
    ...
    test_harness(keystore, move |test_harness| async move {
        ...
        let valid = make_valid_availability_gossip(&test_state, 0, 0);
...
fn add_relay_parent_includes_all_live_candidates() {
    ...
    state
        .per_candidate
        .insert(candidate_hash_b, make_per_candidate());

    let candidates = vec![
        (
            candidate_hash_a,
            FetchedLiveCandidate::Fresh(Default::default()),
        ),
...
fn query_pending_availability_at_pulls_from_and_updates_receipts() {
    ...
    let test_fut = async move {
        let live_candidates =
            query_pending_availability_at(&mut ctx, vec![hash_a, hash_b], &mut receipts)
                .await
                .unwrap();
        ...
        assert_matches!(
            live_candidates.get(&candidate_hash_a).unwrap(),
            FetchedLiveCandidate::Cached
        );
        assert_matches!(
            live_candidates.get(&candidate_hash_b).unwrap(),
            FetchedLiveCandidate::Cached
        );
        assert_matches!(
            live_candidates.get(&candidate_hash_c).unwrap(),
            FetchedLiveCandidate::Fresh(_)
        );
