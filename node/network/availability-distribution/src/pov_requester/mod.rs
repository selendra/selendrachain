...
use indracore_subsystem::{
    ...
    ActivatedLeaf, ActiveLeavesUpdate, SubsystemContext,
};

use crate::{
    error::{log_error, Error},
    runtime::{Runtime, ValidatorInfo},
};
...
pub struct PoVRequester {
    ...
    connected_validators:
        LruCache<SessionIndex, Option<mpsc::Receiver<(AuthorityDiscoveryId, PeerId)>>>,
}
...
impl PoVRequester {
    ...
    {
        let activated = update
            .activated
            .iter()
            .map(|ActivatedLeaf { hash: h, .. }| h);
        let activated_sessions = get_activated_sessions(ctx, runtime, activated).await?;
        ...
        Context: SubsystemContext,
    {
        ...
        let span =
            jaeger::Span::new(candidate_hash, "fetch-pov").with_validator_index(from_validator);
        ctx.spawn(
            "pov-fetcher",
...
async fn do_fetch_pov(
    ...
    let pov = match response {
        ...
        PoVFetchingResponse::NoSuchPoV => return Err(Error::NoSuchPoV),
    };
...
async fn get_activated_sessions<Context>(
    ctx: &mut Context,
    runtime: &mut Runtime,
    new_heads: impl Iterator<Item = &Hash>,
...
where
    Context: SubsystemContext,
{
    if let Some(validator_ids) =
        determine_relevant_validators(ctx, runtime, parent, session).await?
    {
...
{
    let info = runtime
        .get_session_info_by_index(ctx, parent, session)
        .await?;
                ...
                match virtual_overseer.recv().await {
                    ...
                    AllMessages::NetworkBridge(NetworkBridgeMessage::SendRequests(mut reqs, _)) => {
                        ...
                        req.pending_response
                            .send(Ok(PoVFetchingResponse::PoV(
                                CompressedPoV::compress(&pov).unwrap(),
                            )
                            .encode()))
                            .unwrap();
                        break;
