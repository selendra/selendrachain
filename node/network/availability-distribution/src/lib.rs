...
use indracore_subsystem::{
    ...
    jaeger, ActiveLeavesUpdate, FromOverseer, OverseerSignal, PerLeafSpan, SpawnedSubsystem, Stage,
...
impl PerCandidate {
    ...
    fn message_required_by_peer(&self, peer: &PeerId, validator_index: ValidatorIndex) -> bool {
        self.received_messages
            .get(peer)
            .map(|v| !v.contains(&validator_index))
            .unwrap_or(true)
            && self
                .sent_messages
                .get(peer)
...
impl ProtocolState {
    ...
    fn cached_live_candidates_unioned<'a>(
        ...
        cached_live_candidates_unioned(&self.per_relay_parent, relay_parents)
    }
    ...
    fn add_relay_parent(
        ...
        for (receipt_hash, fetched) in candidates {
            let candidate_entry = match self.per_candidate.entry(receipt_hash) {
                ...
                Entry::Vacant(e) => {
                    if let FetchedLiveCandidate::Fresh(descriptor) = fetched {
                        ...
                        tracing::warn!(
                            target: LOG_TARGET,
                            "No `per_candidate` but not fresh. logic error"
                        );
                        continue;
                    }
                }
            ...
            let span = per_relay_parent
                .span
                .child_builder("live-candidate")
    ...
    fn clean_up_live_under_cache(&mut self) {
        let extended_view: HashSet<_> = self
            .per_relay_parent
            .iter()
        ...
        self.live_under
            .retain(|ancestor_hash, _| extended_view.contains(ancestor_hash));
    }
}
    ...
    Context: SubsystemContext<Message = AvailabilityDistributionMessage>,
{
    ...
    old_view
        .difference(&view)
        .for_each(|r| state.remove_relay_parent(r));
...
async fn send_batch_to_network(
    ...
    if !batch.is_empty() {
        ctx.send_message(NetworkBridgeMessage::SendValidationMessages(batch).into())
            .await
    }
}
        ...
        let messages = ((0 as ValidatorIndex)..(per_candidate.validators.len() as ValidatorIndex))
            ...
            .filter_map(|erasure_chunk_index: ValidatorIndex| {
                ...
                per_candidate
                    ...
                    .filter(|_| {
                        per_candidate.message_required_by_peer(&origin, erasure_chunk_index)
                    })
            })
            .cloned()
            .collect::<HashSet<_>>();
    ...
    Context: SubsystemContext<Message = AvailabilityDistributionMessage>,
{
    ...
    if let Some(existing) = candidate_entry
        .message_vault
        .get(&message.erasure_chunk.index)
    {
        let span = candidate_entry
            .span
            .child_with_candidate("handle-duplicate", &candidate_hash);
    ...
    let peers = {
        ...
        state
            ...
            .filter(|(_, view)| {
                ...
                cached_live_candidates_unioned(per_relay_parent, view.heads.iter())
                    .contains(&message.candidate_hash)
            })
            .map(|(peer, _)| -> PeerId { peer.clone() })
            .filter(|peer| {
                candidate_entry.message_required_by_peer(peer, message.erasure_chunk.index)
            })
            .collect::<Vec<_>>()
    };
            ...
            Entry::Occupied(e) => {
                live_candidates.extend(
                    e.get()
                        .iter()
                        .cloned()
                        .map(|c| (c, FetchedLiveCandidate::Cached)),
                );
                continue;
            }
            e => e.or_default(),
    ...
    Context: SubsystemContext<Message = AvailabilityDistributionMessage>,
{
    ...
    while let Some((ancestor, ancestor_parent)) =
        iter.next().and_then(|a| iter.peek().map(|ap| (a, ap)))
    {
