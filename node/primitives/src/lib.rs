...
impl std::convert::TryFrom<FromTableMisbehavior> for MisbehaviorReport {
    ...
    fn try_from(f: FromTableMisbehavior) -> Result<Self, Self::Error> {
        match f.report {
            TableMisbehavior::ValidityDoubleVote(TableValidityDoubleVote::IssuedAndValidity(
                (c, s1),
                (d, s2),
            )) => {
                let receipt = c.clone();
                ...
                Ok(MisbehaviorReport::SelfContradiction(
                    receipt.to_plain(),
                    signed_1,
                    signed_2,
                ))
            }
            TableMisbehavior::ValidityDoubleVote(TableValidityDoubleVote::IssuedAndInvalidity(
                (c, s1),
                (d, s2),
            )) => {
                let receipt = c.clone();
            ...
            TableMisbehavior::MultipleCandidates(TableMultipleCandidates { first, second }) => {
                let signed_1 = SignedFullStatement::new(
