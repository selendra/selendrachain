...
mod tests {
    ...
    fn forward_subsystem_works() {
        ...
        let all_subsystems =
            AllSubsystems::<()>::dummy().replace_candidate_selection(ForwardSubsystem(tx));
        let (overseer, mut handler) =
            Overseer::new(Vec::new(), all_subsystems, None, spawner.clone()).unwrap();

        spawner.spawn("overseer", overseer.run().then(|_| async { () }).boxed());

        block_on(handler.send_msg(CandidateSelectionMessage::Invalid(
            Default::default(),
            Default::default(),
        )));
        assert!(matches!(
            block_on(rx.into_future()).0.unwrap(),
            CandidateSelectionMessage::Invalid(_, _)
        ));
    }
}
