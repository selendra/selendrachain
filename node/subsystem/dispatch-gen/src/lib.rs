...
use std::fmt;
use syn::{
    parse2, Error, Fields, FieldsNamed, FieldsUnnamed, Ident, ItemEnum, Path, Result, Type, Variant,
};

#[proc_macro_attribute]
pub fn subsystem_dispatch_gen(
    ...
    impl_subsystem_dispatch_gen(attr.into(), item)
        .unwrap_or_else(|err| {
...
fn prepare_enum_variant(variant: &mut Variant) -> Result<EnumVariantDispatch> {
        ...
        match variant.fields.clone() {
            ...
            Fields::Named(FieldsNamed {
                ...
                .ok_or_else(|| {
                    Error::new(
                        span,
                        "To dispatch with struct enum variant, one element must named `inner`",
                    )
                })?,
            ...
            Fields::Unnamed(FieldsUnnamed {
                ...
                .ok_or_else(|| {
                    Error::new(
                        span,
                        "Must be annotated with skip, even if no inner types exist.",
                    )
                })?,
            _ if skip => None,
            Fields::Unit => return Err(Error::new(span, "Must be annotated with #[skip].")),
            Fields::Unnamed(_) => return Err(Error::new(
                span,
    ...
    Ok(EnumVariantDispatch {
        name: variant.ident,
        inner,
    })
}

fn impl_subsystem_dispatch_gen(
    attr: TokenStream,
    item: TokenStream,
    ...
    let variants = ie.variants.iter_mut().try_fold(
        Vec::<EnumVariantDispatchWithTy>::new(),
        |mut acc, variant| {
            let variant = prepare_enum_variant(variant)?;
            if variant.inner.is_some() {
                acc.push(EnumVariantDispatchWithTy {
                    ty: message_enum.clone(),
                    variant,
                })
            }
            Ok::<_, syn::Error>(acc)
...
mod tests {
    ...
    fn basic() {
        ...
        let output =
            impl_subsystem_dispatch_gen(attr, item).expect("Simple example always works. qed");
        println!("//generated:");
        println!("{}", output);
    }
