...
pub async fn forward_events<P: BlockchainEvents<Block>>(client: Arc<P>, mut handler: Handle) {
	...
	S: SpawnNamed,
{
	...
	async fn stop(mut self) {
		let _ = self
			.wait_terminate(OverseerSignal::Conclude, ::std::time::Duration::from_secs(1_u64))
			.await;
	}
	...
	pub async fn run(mut self) -> SubsystemResult<()> {
		...
		for (hash, number) in std::mem::take(&mut self.leaves) {
			...
			if let Some((span, status)) = self.on_head_activated(&hash, None) {
				update.activated.push(ActivatedLeaf { hash, number, status, span });
			}
		}
