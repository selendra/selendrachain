...
use indracore_subsystem::messages::{
    ...
    BitfieldSigningMessage, CandidateBackingMessage, CandidateSelectionMessage,
    CandidateValidationMessage, ChainApiMessage, CollationGenerationMessage,
    CollatorProtocolMessage, NetworkBridgeMessage, PoVDistributionMessage, ProvisionerMessage,
...
impl<M: Send + 'static> SubsystemContext for OverseerSubsystemContext<M> {
    ...
    async fn recv(&mut self) -> SubsystemResult<FromOverseer<M>> {
        self.rx.next().await.ok_or(SubsystemError::Context(
            "No more messages in rx queue to process".to_owned(),
        ))
    }
    ...
    async fn spawn(
        ...
        self.send_timed(ToOverseer::SpawnJob { name, s })
    ...
    async fn spawn_blocking(
        ...
        self.send_timed(ToOverseer::SpawnBlockingJob { name, s })
            .map_err(|s| s.into_send_error().into())
    }
...
impl<M> OverseerSubsystemContext<M> {
    ...
    fn send_timed(
        &mut self,
        msg: ToOverseer,
    ) -> Result<(), mpsc::TrySendError<MaybeTimed<ToOverseer>>> {
...
impl Metrics {
    ...
    fn time_message_hold(&self) -> MaybeTimer {
        self.0
            .as_ref()
            .map(|metrics| metrics.message_relay_timings.start_timer())
    }

    fn channel_fill_level_snapshot(&self, from_overseer: usize, to_overseer: usize) {
        self.0.as_ref().map(|metrics| {
            metrics
                .to_overseer_channel_queue_size
                .set(to_overseer as u64)
        });
        self.0.as_ref().map(|metrics| {
            metrics
                .from_overseer_channel_queue_size
                .set(from_overseer as u64)
        });
    }
}
    ...
    S: SpawnNamed,
{
            ...
            let metronome =
                Metronome::new(std::time::Duration::from_millis(950)).for_each(move |_| {
                    metronome_metrics.channel_fill_level_snapshot(
                        meter_from_overseer.queue_count(),
                        meter_to_overseer.queue_count(),
                    );
    ...
    async fn broadcast_signal(&mut self, signal: OverseerSignal) -> SubsystemResult<()> {
        self.candidate_validation_subsystem
            .send_signal(signal.clone())
            .await?;
        self.candidate_backing_subsystem
            .send_signal(signal.clone())
            .await?;
        self.candidate_selection_subsystem
            .send_signal(signal.clone())
            .await?;
        self.statement_distribution_subsystem
            .send_signal(signal.clone())
            .await?;
        self.availability_distribution_subsystem
            .send_signal(signal.clone())
            .await?;
        self.availability_recovery_subsystem
            .send_signal(signal.clone())
            .await?;
        self.bitfield_signing_subsystem
            .send_signal(signal.clone())
            .await?;
        self.bitfield_distribution_subsystem
            .send_signal(signal.clone())
            .await?;
        self.provisioner_subsystem
            .send_signal(signal.clone())
            .await?;
        self.pov_distribution_subsystem
            .send_signal(signal.clone())
            .await?;
        self.runtime_api_subsystem
            .send_signal(signal.clone())
            .await?;
        self.availability_store_subsystem
            .send_signal(signal.clone())
            .await?;
        self.network_bridge_subsystem
            .send_signal(signal.clone())
            .await?;
        self.chain_api_subsystem.send_signal(signal.clone()).await?;
        self.collator_protocol_subsystem
            .send_signal(signal.clone())
            .await?;
        self.collation_generation_subsystem
            .send_signal(signal.clone())
            .await?;
        self.approval_distribution_subsystem
            .send_signal(signal)
            .await?;
...
fn spawn<S: SpawnNamed, M: Send + 'static>(
    ...
    *seed += 1;
    ...
    let instance = Some(SubsystemInstance { tx: to_tx, name });
...
mod tests {
    ...
    fn overseer_works() {
        ...
        executor::block_on(async move {
            ...
            let (overseer, mut handler) =
                Overseer::new(vec![], all_subsystems, None, spawner).unwrap();
            let overseer_fut = overseer.run().fuse();
    ...
    fn overseer_metrics_work() {
        ...
        executor::block_on(async move {
            ...
            let (overseer, mut handler) =
                Overseer::new(vec![first_block], all_subsystems, Some(&registry), spawner).unwrap();
            let overseer_fut = overseer.run().fuse();
    ...
    fn overseer_ends_on_subsystem_exit() {
        ...
        executor::block_on(async move {
            let all_subsystems =
                AllSubsystems::<()>::dummy().replace_candidate_backing(ReturnOnStart);
            let (overseer, _handle) = Overseer::new(vec![], all_subsystems, None, spawner).unwrap();
    ...
    fn overseer_start_stop_works() {
        ...
        executor::block_on(async move {
            ...
            let (overseer, mut handler) =
                Overseer::new(vec![first_block], all_subsystems, None, spawner).unwrap();
    ...
    fn do_not_send_empty_leaves_update_on_block_finalization() {
        ...
        executor::block_on(async move {
            ...
            let (overseer, mut handler) =
                Overseer::new(Vec::new(), all_subsystems, None, spawner).unwrap();
    ...
    fn overseer_all_subsystems_receive_signals_and_messages() {
        ...
        executor::block_on(async move {
            ...
            handler
                ...
                .await;
            handler
                .send_msg(AllMessages::CandidateSelection(
                    test_candidate_selection_msg(),
                ))
                .await;
            handler
            ...
            handler
                ...
                .await;
            handler
                .send_msg(AllMessages::AvailabilityDistribution(
                    test_availability_distribution_msg(),
                ))
                .await;
            handler
