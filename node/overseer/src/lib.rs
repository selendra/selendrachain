...
pub use indracore_subsystem::{
    ActiveLeavesUpdate, DummySubsystem, FromOverseer, OverseerSignal, SpawnedSubsystem, Subsystem,
    ...
    S: SpawnNamed,
{
    ...
    async fn block_finalized(&mut self, block: BlockInfo) -> SubsystemResult<()> {
        ...
        self.broadcast_signal(OverseerSignal::BlockFinalized(block.hash))
            .await?;
...
fn spawn<S: SpawnNamed, M: Send + 'static>(
    ...
    let ctx = OverseerSubsystemContext {
        rx: to_rx,
        tx: from_tx,
    };
    ...
    let instance = Some(SubsystemInstance { tx: to_tx });
