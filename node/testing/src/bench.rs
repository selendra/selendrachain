...
impl<'a> Iterator for BlockContentIterator<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((sender, signed_extra(0, node_indracore_runtime::ExistentialDeposit::get() + 1))),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive => {
						Call::Balances(
							BalancesCall::transfer_keep_alive(
								pallet_indices::address::Address::Id(receiver),
								node_indracore_runtime::ExistentialDeposit::get() + 1,
							)
						)
					},
					BlockType::RandomTransfersReaping => {
						Call::Balances(
							BalancesCall::transfer(
								...
								100*DOLLARS - (node_indracore_runtime::ExistentialDeposit::get() - 1),
							)
						)
					},
					BlockType::Noop => {
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> node_indracore_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			false,
			Some(node_indracore_runtime::wasm_binary_unwrap()),
